...
button_list = [
    ("Productivity", color_productivity),
    ("Dashboard", color_dashboard),
    ("Data Science", color_ds),
    ("Coding Best Practices", color_best_practices),
    ("Software Engineering", color_se),
]
...
def articles_tab():
    ...
    for idx, values in enumerate(card_list):
        topic, title, description, colour, link = values
        card_div_list.append(
            html.A(
                dbc.Card(
                    ...
                    className=f"zoom background-{colour}",
                ),
    ...
    button_div_list = []
    for topic, colour in button_list:
